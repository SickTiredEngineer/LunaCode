		...
		<div class="page-base container">
			...
			<c:choose>
				<c:when test="${isModify}">
					<c:set var="formAction" value="ModifySupportForm"/>
					<c:set var="submitLabel" value="수정 완료"/>
					<c:set var="pageTitle" value="1:1문의 수정"/>
					
					<c:set var="querySubject" value="${cqvo.query_subject}"/>
					<c:set var="queryContent" value="${cqvo.query_content}"/>
					<c:set var="category" value="${cqvo.query_category}"/>
				</c:when>
				
				<c:otherwise>
					<c:set var="formAction" value="SupportWriteForm"/>
					<c:set var="submitLabel" value="등록"/>
					<c:set var="pageTitle" value="1:1문의 작성"/>
				</c:otherwise>
			</c:choose>
			...
			<article>
				...
				<div class="d-flex flex-row">
					...
					<div class="d-flex flex-row">
						...
						<form action="${formAction}" id="supportForm" method="post" class="d-flex flex-column">
							...
							<div class="d-flex flex-column write_main_layout">
								...
								<div class="d-flex flex-row write-layout">
									...
									<input type="text" placeholder="제목" name="query_subject" class="common-input-form form-size" value="${querySubject}">
								</div>
								...
								<div class="d-flex flex-row write-layout">
									...
									<select  name="query_category" class="category info-text">
								...
								<div class="d-flex flex-row write-layout-content">
										...
										<textarea rows="10" placeholder="내용" cols="50" name="query_content" class="common-input-form">${queryContent}</textarea>
								</div>
